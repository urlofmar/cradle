...
// Test that a type correctly implements the CRADLE Regular type interface for the given
// value. Note that this only tests the portions of the interface that are relevant for
// a single value. (In particular, it's intended to work for types that only have one
// possible value.)
template<class T>
void
...
// Test that a type correctly implements the CRADLE Regular type interface for the given
// pair of values. This does additional tests that require two different values. It
// assumes that :x < :y and that the two produce different hash values.
template<class T>
void
test_regular_value_pair(T const& x, T const& y)
{
    ...
    // :test_pair accepts a second pair of values (:a, :b) which is meant to match the
    // original pair (:x, :y). It tests that this is true and also tests the original
