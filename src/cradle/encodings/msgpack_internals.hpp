...
// Buffer concept and initialize the msgpack::packer object. Thus, you can potentially use
// this for streaming and other more interesting forms of I/O.
//
// Note that because this must include the msgpack-c header, it indirectly includes all sorts of
// other stuff, includings windows.h on Windows, so use with caution.
     ...
     case value_type::BLOB:
      {
        ...
        // Check to make sure that the blob size is within the MessagePack specification's
        // limit.
     ...
     case value_type::DATETIME:
      {
        ...
        int64_t t = (cast<ptime>(v) - ptime(date(1970, 1, 1))).total_milliseconds();
        ...
        else if (t >= -int64_t(0x80'00'00'00) && t < int64_t(0x80'00'00'00))
        {
            int32_t x = int32_t(t);
            boost::endian::native_to_big_inplace(x);
