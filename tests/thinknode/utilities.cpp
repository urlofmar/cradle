...
TEST_CASE("Thinknode type conversion", "[thinknode][utilities]")
{
    ...
    auto array_type = make_api_type_info_with_array(
        make_api_array_info(boolean_type, none));
    ...
    auto map_type
        = make_api_type_info_with_map(make_api_map_info(array_type, blob_type));
    ...
    auto struct_type = make_api_type_info_with_structure(
        {{"def", make_api_structure_field_info("ijk", none, array_type)},
         {"abc", make_api_structure_field_info("xyz", true, blob_type)}});
