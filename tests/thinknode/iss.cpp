...
TEST_CASE("URL type string", "[thinknode]")
{
    ...
    auto named_type = construct_api_type_info_with_named_type(named_info);
    REQUIRE(get_url_type_string(named_type) == "named/my_account/my_app/my_type");

    auto integer_type = construct_api_type_info_with_integer_type(api_integer_type());
    REQUIRE(get_url_type_string(integer_type) == "integer");

    auto float_type = construct_api_type_info_with_float_type(api_float_type());
    REQUIRE(get_url_type_string(float_type) == "float");

    auto string_type = construct_api_type_info_with_string_type(api_string_type());
    REQUIRE(get_url_type_string(string_type) == "string");

    auto boolean_type = construct_api_type_info_with_boolean_type(api_boolean_type());
    REQUIRE(get_url_type_string(boolean_type) == "boolean");

    auto blob_type = construct_api_type_info_with_blob_type(api_blob_type());
    REQUIRE(get_url_type_string(blob_type) == "blob");

    auto dynamic_type = construct_api_type_info_with_dynamic_type(api_dynamic_type());
    REQUIRE(get_url_type_string(dynamic_type) == "dynamic");

    auto nil_type = construct_api_type_info_with_nil_type(api_nil_type());
    REQUIRE(get_url_type_string(nil_type) == "nil");

    auto datetime_type = construct_api_type_info_with_datetime_type(api_datetime_type());
    REQUIRE(get_url_type_string(datetime_type) == "datetime");

    api_array_info array_info;
    array_info.element_schema = boolean_type;
    auto array_type = construct_api_type_info_with_array_type(array_info);
    ...
    auto map_type = construct_api_type_info_with_map_type(map_info);
    ...
    auto struct_type = construct_api_type_info_with_structure_type(struct_info);
    ...
    auto union_type = construct_api_type_info_with_union_type(union_info);
    REQUIRE(get_url_type_string(union_type) == "union/3/abc/blob/def/array/boolean/ghi/string");

    auto optional_type = construct_api_type_info_with_optional_type(map_type);
    REQUIRE(get_url_type_string(optional_type) == "optional/map/array/boolean/blob");

    api_enum_info enum_info;
    enum_info.values["def"] = api_enum_value_info();
    enum_info.values["abc"] = api_enum_value_info();
    auto enum_type = construct_api_type_info_with_enum_type(enum_info);
    REQUIRE(get_url_type_string(enum_type) == "enum/2/abc/def");

    auto ref_type = construct_api_type_info_with_reference_type(named_type);
...
TEST_CASE("ISS POST", "[thinknode]")
{
    ...
    auto id =
        post_iss_object(
            ...
            session,
            "123",
            construct_api_type_info_with_string_type(api_string_type()),
            value("payload"));
    REQUIRE(id == "def");
}
